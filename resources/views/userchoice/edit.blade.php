...
@section('contentz')
    <div class="card" style="margin: 20px;">
        ...
        <div class="card-body">
            <form action="{{ route('crud.userchoice.update', ['id' => $userchoice->id]) }}" method="post">
                @csrf
                @method('PUT')
...
<option value="{{ $Variant_Item->id }}" data-item="{{ $Variant_Item->variance_id }}" class="Variant_Item-option" >
...
const showVariantCheckbox = document.getElementById('showVariant');
const variantContainer = document.getElementById('variantContainer');

showVariantCheckbox.addEventListener('change', function () {

variantContainer.style.display = this.checked ? 'block' : 'none';
});
</script>
@endsection